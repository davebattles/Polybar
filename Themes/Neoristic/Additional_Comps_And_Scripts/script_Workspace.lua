...
function ChangeWorkspaceAnimation()
	if timing > 0 and timing < maxTime then
		...
		if currentWorkspace <= maximumWorkspace then 
			...
			SKIN:Bang('!SetOPtion', 'WorkspaceCurrent', 'Shape', 'Rectangle ('..slideAnimation..'),0,#Workspace_Number_Gap#,#Bar_Height# | Extend BackgroundTrait')
			SKIN:Bang('!SetOPtion', 'WorkspaceCurrent', 'Shape2', 'Rectangle ('..slideAnimation..'),#Bar_Height#,#Workspace_Number_Gap#,-3 | Extend IndicatorTrait')
